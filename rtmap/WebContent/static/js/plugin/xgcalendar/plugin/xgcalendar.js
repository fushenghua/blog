...
; (function($) {
    ...
    $.fn.bcalendar = function(option) {
        ...
        function parseJsonDate(sD1){
             ...
             var sHour = sDstr[3];
             var sMinute = sDstr[4];
             var sD = new Date(sYear,sMonth - 1,sDate,sHour,sMinute);
             return sD;
        }
...
//                        datastr =datastr.replace(/"\\\/(Date\([0-9-]+\))\\\/"/gi, 'new $1');
                        ...
                        data.start = parseJsonDate(data.start);
                        data.end = parseJsonDate(data.end);
                        for(var ii = 0; ii < data.events.length ; ii++){
                        	var event = data.events[ii]; 
                        	event[2] = parseJsonDate(event[2]);
                        	event[3] = parseJsonDate(event[3]);
                        }
                        //debugger;
...
//                    	+ i18n.xgcalendar.update_detail + ' <StrONG>»</StrONG>
